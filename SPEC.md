...
QuizMD supports **Penrose** declarative mathematical diagrams, rendered via [@penrose/core](https://penrose.cs.cmu.edu/) in compatible renderers. Diagrams may appear in question bodies and feedback.

### Syntax

Use a fenced code block with the `penrose` language identifier. The block is divided into **three sections** separated by a line containing exactly `---`:
...
- **2 sections** (`style / substance`): domain is empty
- **1 section** (`substance` only): domain and style are both empty

Penrose is **auto-detected**: `@penrose/core` is loaded only when a ` ```penrose ` block is present.
...
type Point
type Segment
constructor MkSeg(Point p1, Point p2) -> Segment
---
...
Point p {
  ...
  p.label above p.icon
}

Segment s {
  shape s.line = Line {
    start: s.p1.icon.center
    end: s.p2.icon.center
...
- The full Penrose language reference is available at [penrose.cs.cmu.edu](https://penrose.cs.cmu.edu/).
- Diagrams requiring external font files may not render text labels in all environments.
- Complex diagrams with many objects may take a moment to optimize.
